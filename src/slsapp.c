...
static void
setup_gl_attributes(slsApp* self)
{
...
sls_create_app(slsApp* self)
{
  *self = (slsApp){ 0 };
  ...
  self->renderer = sls_create_renderer(
    malloc(sizeof(slsRenderer)), self->window, self->ctx, &result);
  sls_check(result == SLS_OK,
            "create_renderer failed: %s",
...
handle_windowevent(slsApp* self, SDL_WindowEvent const* windowevent)
{
  switch (windowevent->event) {
    case SDL_WINDOWEVENT_RESIZED:
      ...
      sls_renderer_onresize(
        self->renderer, windowevent->data1, windowevent->data2);
      break;
    default:
      break;
  }
...
sls_app_update(slsApp* self, double dt)
{
  ...
  for (int i = 0; i < world->length; ++i) {
    ...
    if ((mask & (SLS_COMPONENT_MOVEMENT | SLS_COMPONENT_PLAYERCONTROLLED)) !=
        0) {
