...
void create_sprite_buffers(slsRenderer *self);

void create_grid_geom(struct slsGridParams const *params, uint32_t *indices,
                      slsVertex *verts);
                                 ...
                                 slsResultCode *result_out)
{
  ...
  *self = (slsRenderer) {.clear_color={1.0, 0.0, 1.0, 1.0}};
  ...
  struct slsGridParams grid_params = {
      .n_cols=10,
      .n_rows=10,
      .col_size=1.0,
      .row_size=1.0,
  ...
  self->grid = sls_create_grid(malloc(sizeof(slsGrid)), &grid_res, &grid_params);
  if (grid_res != SLS_OK) {
    free(self->grid);
...
void sls_renderer_onresize(slsRenderer *self, int width, int height)
{
  ...
  mat4x4_ortho(self->main_camera.projection.m, 0, (float) width / 8.f, 0,
               (float) height / 8.f, 0.0, 100.0);
}
...
void sls_renderer_draw_sprite(slsRenderer *self, float rotation_theta)
{
  ...
  GLint projection_id = glGetUniformLocation(self->sprite_program, "projection");
...
void sls_render_sprite_system(slsRenderer *self, slsEntityWorld *world)
{
  ...
  for (int i = 0; i < world->length; ++i) {
    if ((world->masks[i] & SLS_SYSTEM_RENDERSPRITE) !=
        SLS_SYSTEM_RENDERSPRITE) { continue; }
    n_entities++;
...
static inline void setup_buffers_layout(slsRenderBuffers *self)
{
...
static const struct slsGridParams default_params = {
    .origin={0.f, 0.f, 0.f},
    .row_size=1.f,
    .col_size=1.f,
    .n_rows=10,
                         ...
                         struct slsGridParams const *params)
{
  ...
  *self = (slsGrid) {.params=*params, .shader_program=program};
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(slsVertex) * n_vertices, verts, GL_STATIC_DRAW);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(uint32_t) * n_vertices, indices,
               GL_STATIC_DRAW);
  glBindVertexArray(0);
                      ...
                      slsVertex *verts)
{
  ...
  for (int i = 0; i < params->n_cols; ++i) {
    ...
    verts[index] = (slsVertex) {
        .position={x, row_size_half, 0.f},
        .color={0.4, 0.4, 0.4}
    };
