...
slsMesh *slsMesh_alloc()
{
	...
	mesh->bindTransform = slsMesh_bindTransform;
...
slsMesh *slsMesh_init(slsMesh *self)
{
	...
	self->transform = slsMat4f_identity();
...
{
	sls_push_matrix_unif(&(self->transform), model);
}
...
void slsMesh_draw(slsMesh *self, slsShader *shader, SLenum draw_type)
{
	...
	slsMsg(self, bindTransform, shader->uniforms.model);
